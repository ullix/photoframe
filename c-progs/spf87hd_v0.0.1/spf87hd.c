...
int find_spf()
{
 ...
 if (cnt > 0)
 {
  ...
  { //found device
   ...
   // sequence 1/9
/*    if ( opmode == NODEAMON )
     fprintf(stderr, "Sending cmd seq %i.\n", 1);
    else
     syslog(FACILITY,"Sending cmd seq %i.\n", 1);
*/
...
int cmd_start(const char *usrname, const char *argment)
{
 if (system(NULL) == 0)
 {
 ...
 {
  char cmd[80]="";
  sprintf(cmd, "sudo -b -n -u %s -i %s", usrname, argment);
  if ((system(cmd)) == -1)
  {
   if (opmode == NODEAMON)
...
int main(int argc, char **argv)
{
 ...
 int option_index = 0;
 int opt = getopt_long (argc, argv, "fm:n:s:u:h", long_options, &option_index);
 ...
 while (opt != -1)
 {
  ...
  opt = getopt_long( argc, argv, "fm:n:s:u:h", long_options, &option_index );
 }
 if (optind < argc)
 {
 ...
 if ((!username) && ((monitor_cmd) || (desktop_cmd)))
 {
  fprintf(stderr, "monitor_cmd %s and/or desktop_cmd %s set without username\n", monitor_cmd, desktop_cmd);
  exit(1);
 }
 if (opmode == DEAMON)
 {
 ...
 {
  if (opmode == NODEAMON)
   fprintf(stderr, "can't open /proc/bus/usb/devices\n");
  else
   syslog(FACILITY,"can't open /proc/bus/usb/devices\n");
 }
 int ret = grepOnce(stream, PATTERN);
 fclose(stream);
 if(ret)
 {
  ...
  if(CMD_SENT==-1)
  {
   ...
   if (CMD_SENT == 1 )
    if (monitor_cmd)
     if (( cmd_start(username,monitor_cmd)) == 1)
     {
      if ( opmode == NODEAMON )
 ...
 else
 {
  if ( CMD_SENT == 1 )
  {
   ...
   if (desktop_cmd)
   {
    if ((cmd_start(username,desktop_cmd)) == 1)
    {
     if ( opmode == NODEAMON )
